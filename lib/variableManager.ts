...
import { CollectionInfo, PluginError, VariableCacheEntry } from './types';
import { PLUGIN_CONFIG, ERROR_CODES } from './constants';
import { preprocessTextForVariable } from './textProcessor';

export async function getVariableCollections(): Promise<CollectionInfo[]> {
  try {
...
export async function createDefaultCollection(): Promise<string> {
  try {
...
export async function validateCollection(collectionId: string): Promise<VariableCollection> {
...
export async function getExistingVariables(collectionId: string): Promise<Map<string, Variable>> {
  try {
...
export async function createStringVariable(
  collectionId: string, 
  variableName: string, 
  content: string
): Promise<Variable> {
...
export function bindTextNodeToVariable(textNode: TextNode, variable: Variable): void {
  try {
...
export function findExistingVariable(
...
export function addToVariableCache(
...
export function getFromVariableCache(
  cache: Map<string, VariableCacheEntry>,
  variableName: string,
  content: string
): Variable | null {
...
export async function processTextLayerToVariable(
  textLayer: TextNode,
  collectionId: string,
  existingVariables: Map<string, Variable>,
  variableCache: Map<string, VariableCacheEntry>
): Promise<{ variable: Variable; wasCreated: boolean }> {
